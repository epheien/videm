...
if !exists('g:useTagsStorage')
	let g:useTagsStorage = 1
endif
...
function! s:TagPopupList(szTypeName, szBase)
    ...
    if a:szTypeName!=''
        " Scope search
		" 搜索属于指定 socpe 的成员(eg. DFoo::DFoo, DFoo::~DFoo, DFoo::m_foo)
        let szTagQuery = '^' . a:szTypeName . '::' . a:szBase . '\~\?\w\+$'
    else
        " Global search
		" 搜索匹配指定前缀的在全局作用域中的符号(eg. pri -> printf )
    ...
    try
        " Getting tags
		" TODO: 使用 TagsStorage 获取结果
		if g:useTagsStorage
			let l:scope = a:szTypeName
			if l:scope == ''
				"作用域为全局
				let l:scope = '<global>'
			endif
			let l:name = a:szBase
			let result = g:GetTagsByScopeAndName(l:scope, l:name)
			let b:tagResult = copy(result)
		else
...
function! s:SearchGlobalMembers(szBase)
    if a:szBase != ''
		...
		"过滤所有被包含在 namespace, class, struct, union 的条目?!
...
function! omni#cpp#complete#Main(findstart, base)
    if a:findstart
        ...
        " Get contexts stack, 上下文(作用域)栈(列表) eg. ['::std', '::MyNS', '::']
    ...
    if len(g:omni#cpp#items#data)==0
        ...
        for szCurrentContext in s:contextStack
            ...
            let resolvedTagItem = omni#cpp#utils#GetResolvedTagItem(
						\s:contextStack, 
						\omni#cpp#utils#CreateTypeInfo(szCurrentContext))
            if resolvedTagItem != {}
		...
		" typeinfo 标识是否需要再解析
		" 如不需再解析, 值类似 {'type': 1, 'value': 'DFoo'}
		" 如需再解析, 值类似 {'type': 4, 'value': tag} (tag 为标准tag字典)
		" g:omni#cpp#items#data 为光标前的 tokens 解析结果
		" eg. dfoo.testAnonStruct. 结果为
		" [{'tokens': 
		" [{'kind': 'cppWord', 'value': 'dfoo'}], 
		" 'kind': 'itemVariable'}, 
		" {'tokens': 
		" [{'kind': 'cppWord', 'value': 'testAnonStruct'}], 
		" 'kind': 'itemVariable'}]
        ...
        if typeInfo != {}
            if g:omni#cpp#items#data[-1].kind == 'itemScope'
				...
				" omni#cpp#utils#GetResolvedTags()
				" 负责解析类(包括继承关系)/结构/联合的 scope
				" 返回需要搜索的 scope 的列表
				" DFoo : Foo, Foo2 -> 返回类名为 DFoo, Foo, Foo2 的 tag 字典
                for resolvedTagItem in omni#cpp#utils#GetResolvedTags(s:contextStack, typeInfo)
					" szTypeInfo 为完整作用域, 如 ::DFoo
                    let szTypeInfo = omni#cpp#utils#ExtractTypeInfoFromTag(resolvedTagItem)
                    if index(s:contextStack, szTypeInfo)<0
                        let szAccessFilter = 'public'
                    else
					...
					" s:SearchMembers()
					" 根据上述类名作为作用域获取类的所有成员
					" FIXME: 构造函数被过滤了(eg: DFoo::DFoo -> None)
