...
function! omni#cpp#utils#GetResolvedTagItem(namespaces, typeInfo)
    let typeInfo = {} "eg: {'type': 1, 'value': '::MyClass'}
    if type(a:typeInfo) == type('') "如果是字符串，就要现做 typeInfo
...
function! omni#cpp#utils#IsTypeInfoValid(typeInfo)
    if a:typeInfo=={}
        ...
        if a:typeInfo.type == type('') && a:typeInfo.value==''
            " String case
            return 0
        elseif a:typeInfo.type == type({}) && a:typeInfo.value=={}
            " Dictionary case
            return 0
        endif
    endif
