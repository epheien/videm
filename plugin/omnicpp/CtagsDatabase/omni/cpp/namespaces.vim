...
function! omni#cpp#namespaces#GetMapFromCurrentBuffer()
    ...
    let curPos = [1,1]
    while curPos != [0,0]
		"FIXME: 如果此语句不是在行首呢？过于复杂，暂时为简单的实现
...
function! omni#cpp#namespaces#GetUsingNamespaces()
    ...
    " 1) Firstly, we have to get local using namespace declarations
    let originalPos = getpos('.')
    let origPos = originalPos[1:2]
    ...
    let nextStopLine = origPos[0]
    while curPos !=[0,0]
		" 搜索局部与某作用域(非作用于整个源文件)的 'using namespace' 声明
...
function! s:ResolveNamespace(namespace, mapCurrentContexts)
    ...
    if has_key(g:omni#cpp#namespaces#CacheResolve, a:namespace)
		...
		if g:useTagsStorage
			" 搜索名空间
			let listTagsOfNamespace = g:GetTagsByPath(a:namespace)
		else
...
function! omni#cpp#namespaces#GetClassScopeAtCursor()
	return s:GetClassScopeAtCursor()
endfunc
...
function! omni#cpp#namespaces#GetContexts()
    ...
    if g:OmniCpp_NamespaceSearch && &filetype != 'c'
		...
		" 以搜索方式获取所有 using namespace 语句, 从而获取结果
